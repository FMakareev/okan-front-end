...
export class TextFieldArray extends Component {
  ...
  get initialState() {
    if (this.props.input.value.length) {
      ...
      return {
        selectedOption: options && options.filter(optionItem =>
  ...
  render() {
    const {options, input: {value}, valueKey} = this.props;

    return (<Box width={'100%'}>

      {
        value && Array.isArray(value) && value.map((item, index) =>(<FlexStyled
          py={3}
          key={`FlexStyled-${index}`}
          width={'100%'}
          alignItems={'center'}
        >
