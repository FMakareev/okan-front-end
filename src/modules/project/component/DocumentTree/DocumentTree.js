...
export class DocumentTree extends Component {
  ...
  constructor(props) {
    ...
    this.decorators.Container = props => (
      <SidebarCellNodeWithProject
        ...
        changeActiveNode={id => this.changeActiveNode(id, this.state.tree.children)}
   ...
   * */
  changeRoute = ({ projectid, revisionid, documentid, sectionid }) => {
    try {
  ...
  findNodeByProp = (tree, target, path, prop = 'name') => {
    try {
      ...
      console.error('Error findNodeByProp: ', error);
      captureException(error);
    }
  };
